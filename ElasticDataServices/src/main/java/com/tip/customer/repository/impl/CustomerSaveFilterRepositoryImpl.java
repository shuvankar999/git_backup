...
@Repository
public class CustomerSaveFilterRepositoryImpl implements CustomerSaveFilterRepository{
	
static final Logger logger = LoggerFactory.getLogger(CustomerSaveFilterRepositoryImpl.class);
	...
	public boolean saveFilterDetils(SaveFilterFormRequest saveFilterFormRequest) {		
		...
		try {
			...
			statement = connection.prepareStatement("{call " + ElasticSearchConstant.PROC_CUSTOMER_SAVE_FILTER + "(?,?,?,?,?,?,?)}");
